...
import { Advise, Clockid, Errno, Eventtype, Fdflags, Fdstat, Filestat, Filetype, Fstflags, Lookupflags, Oflags, Prestat, Subscription, Whence, advise, ciovec, clockid, dircookie, dirent, errno, event, exitcode, fd, fdflags, fdstat, filedelta, filesize, filestat, fstflags, iovec, lookupflags, oflags, prestat, rights, subscription, timestamp, whence } from './wasi';
...
export class Memory {
	...
	public readString(ptr: ptr, len: number = -1): string {
		...
		return decodeText((new Uint8Array(this.raw, ptr, length)).slice(0));
	}
...
export namespace TraceMessage {
	export function create(): TraceMessage {
		...
		return {
			...
			path_open: (_memory: ArrayBuffer, result: errno, fd: fd, dirflags: lookupflags, path_ptr: ptr<bytes>, path_len: size, oflags: oflags, _fs_rights_base: rights, _fs_rights_inheriting: rights, fdflags: fdflags, fd_ptr: ptr<fd>): string => {
				...
				if (result === Errno.success) {
					...
					if (result === Errno.success) {
						...
						fileDescriptors.set(resultFd, parentPath !== undefined ? pathJoin(parentPath, path) : path);
					}
					return message;
				} else {
			...
			poll_oneoff: (_memory: ArrayBuffer, result: errno, input: ptr<subscription>, _output: ptr<event[]>, subscriptions: size, _result_size_ptr: ptr<u32>): string => {
				const memory = new DataView(_memory);
				let subscriptionStrs: string[] = [];
				...
				for (let i = 0; i < subscriptions; i++) {
					...
					switch (u.type) {
						case Eventtype.clock:
							subscriptionStrs.push("clock")
							break;
						case Eventtype.fd_read: {
							const fd = subscription.u.fd_read.file_descriptor;
							subscriptionStrs.push(`read ${fd} => ${getFileDescriptorPath(fd)}`)
							break;
						}
						case Eventtype.fd_write: {
							const fd = subscription.u.fd_write.file_descriptor;
							subscriptionStrs.push(`write ${fd} => ${getFileDescriptorPath(fd)}`)
							break;
						}
						default:
							subscriptionStrs.push("unknown");
							break;
					}
					subscription_offset += Subscription.size;
				}

				return `poll_oneoff(${subscriptionStrs.join(", ")}) => [result: ${Errno.toString(result)}]`;
			},
