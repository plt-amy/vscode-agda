...
import { DeviceDriverKind, DeviceId, FileSystemDeviceDriver, NoSysDeviceDriver, ReaddirEntry, WritePermDeniedDeviceDriver } from './deviceDriver';
import { BaseFileDescriptor, FdProvider, FileDescriptor } from './fileDescriptor';
import { LRUCache } from './linkedMap';
import { dirname as pathDirname, join as pathJoin } from "./path";
import {
	Errno, fd, fdflags, Fdflags, fdstat, filesize, filestat, Filetype, filetype, fstflags,
	inode, lookupflags, Lookupflags, oflags, Oflags, Rights, rights, timestamp, WasiError, Whence
} from './wasi';
...
class FileSystem {
	...
	public getUri(node: DirectoryNode, fsPath?: string): Uri {
		const finalPath = fsPath === undefined || fsPath === '.' ? this.getPath(node) : pathJoin(this.getPath(node), fsPath);
		return this.vscfs.with({ path: pathJoin(this.vscfs.path, finalPath) });
	}
	...
	public getOrCreateNode(parent: DirectoryNode, path: string, kind: NodeKind): FileNode | DirectoryNode {
	...
	public getOrCreateNodeOwned(parent: DirectoryNode, path: string, kind: NodeKind): FileNode | DirectoryNode {
		const node = this.getOrCreateNode(parent, path, kind);
		node.refs++;
		return node;
	}
	...
	public renameNode(oldNode: Node, stat: FileStat | undefined, content: Uint8Array | undefined, newParent: DirectoryNode, newPath: string): void {
		...
		this.getOrCreateNode(newParent, newPath, oldNode.kind);
	}
...
export function create(deviceId: DeviceId, baseUri: Uri, readOnly: boolean = false): FileSystemDeviceDriver {
	...
	function createFileDescriptor(parentDescriptor: DirectoryFileDescriptor, fd: fd, rights_base: rights, fdflags: fdflags, path: string): FileFileDescriptor {
		...
		return new FileFileDescriptor(deviceId, fd, rights_base, fdflags, fs.getOrCreateNodeOwned(parentNode, path, NodeKind.File).inode);
	}
	...
	function createDirectoryDescriptor(parentDescriptor: DirectoryFileDescriptor, fd: fd, rights_base: rights, rights_inheriting: rights, fdflags: fdflags, path: string): DirectoryFileDescriptor {
		const parentNode = fs.getNode(parentDescriptor.inode, NodeKind.Directory);
		return new DirectoryFileDescriptor(deviceId, fd, rights_base, rights_inheriting, fdflags, fs.getOrCreateNodeOwned(parentNode, path, NodeKind.Directory).inode);
	}
	...
	const $this: FileSystemDeviceDriver = {
		...
		async fd_readdir(fileDescriptor: FileDescriptor): Promise<ReaddirEntry[]> {
			...
			for (const entry of entries) {
				...
				result.push({ d_ino: fs.getOrCreateNode(directoryNode, name, nodeKind).inode, d_type: filetype, d_name: name });
			}
			return result;
		},
		...
		async path_filestat_get(fileDescriptor: FileDescriptor, _flags: lookupflags, path: string, result: filestat): Promise<void> {
			...
			assignStat(result, fs.getOrCreateNode(inode, path, vStat.type === FileType.Directory ? NodeKind.Directory : NodeKind.File).inode, vStat);
		},
		...
		async path_remove_directory(fileDescriptor: FileDescriptor, path: string): Promise<void> {
			...
			await vscode_fs.delete(fs.getUri(inode, path), { recursive: false });
