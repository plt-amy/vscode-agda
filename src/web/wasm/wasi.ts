...
import {
	ArgumentTransfer, ArgumentsTransfer, Byte, Bytes, CustomMemoryTransfer, MemoryTransferDirection, Ptr,
	S64, SingleReverseArgumentTransfer, Size, U16, U32, U64, U8, WasiFunctionSignature, WasiFunctions
} from './wasiMeta';
...
export namespace Event {
	...
	const offset_userdata = 0;
	const offset_error = 8;
	const offset_type = 10;
	const offset_fd_readwrite = 16;

	const offset_fd_rw_nbytes = offset_fd_readwrite;
	const offset_fd_rw_flags = offset_fd_readwrite + 8;

	export const write = (
		memory: DataView, ptr: ptr, userdata: userdata, type: eventtype, error: errno,
		nbytes: bigint = 0n, flags: eventrwflags = 0
	): void => {
		memory.setBigUint64(ptr + offset_userdata, userdata, true);
		memory.setUint16(ptr + offset_error, error, true);
		memory.setUint8(ptr + offset_type, type);
		memory.setBigUint64(ptr + offset_fd_rw_nbytes, nbytes, true);
		memory.setUint16(ptr + offset_fd_rw_flags, flags, true);
	};
}
...
export namespace Subscription_clock {
	...
	const offset_id = 0;
	const offset_timeout = 8;
	const offset_precision = 16;
	const offset_flags = 24;

	export const getId = (memory: DataView, ptr: ptr): clockid => memory.getUint32(ptr + offset_id, true);
	export const getTimeout = (memory: DataView, ptr: ptr): timestamp => memory.getBigUint64(ptr + offset_timeout, true);
	export const getPrecision = (memory: DataView, ptr: ptr): timestamp => memory.getBigUint64(ptr + offset_precision, true);
	export const getFlags = (memory: DataView, ptr: ptr): subclockflags => memory.getUint16(ptr + offset_flags, true);

	export function create(memory: DataView, ptr: ptr): subscription_clock {
		return {
			get id(): clockid { return memory.getUint32(ptr + offset_id, true); },
			get timeout(): timestamp { return memory.getBigUint64(ptr + offset_timeout, true); },
			get precision(): timestamp { return memory.getBigUint64(ptr + offset_precision, true); },
			get flags(): subclockflags { return memory.getUint16(ptr + offset_flags, true); }
		};
	}
}
...
export namespace Subscription_fd_readwrite {
	...
	const offset_file_descriptor = 0;

	export const get_file_descriptor = (memory: DataView, ptr: ptr): fd => memory.getUint32(ptr + offset_file_descriptor, true);

	export function create(memory: DataView, ptr: ptr): subscription_fd_readwrite {
		return {
			get file_descriptor(): fd { return memory.getUint32(ptr + offset_file_descriptor, true); }
		};
	}
}
...
export namespace Subscription_u {
	...
	export const offset_clock = 8;
	export const offset_fd_read = 8;
	export const offset_fd_write = 8;

	export function create(memory: DataView, ptr: ptr): subscription_u {
		return {
			get type(): eventtype { return memory.getUint8(ptr + offset_type); },
			get clock(): subscription_clock {
				if (memory.getUint8(ptr + offset_type) !== Eventtype.clock) {
					throw new WasiError(Errno.inval);
				}
				return Subscription_clock.create(memory, ptr + offset_clock);
			},
			get fd_read(): subscription_fd_readwrite {
				if (memory.getUint8(ptr + offset_type) !== Eventtype.fd_read) {
					throw new WasiError(Errno.inval);
				}
				return Subscription_fd_readwrite.create(memory, ptr + offset_fd_read);
			},
			get fd_write(): subscription_fd_readwrite {
				if (memory.getUint8(ptr + offset_type) !== Eventtype.fd_write) {
					throw new WasiError(Errno.inval);
				}
				return Subscription_fd_readwrite.create(memory, ptr + offset_fd_write);
			}
		};
	}
}
...
export namespace Subscription {
	...
	export const offset_u = 8;
	const offset_userdata = 0;

	export const getUserdata = (memory: DataView, ptr: ptr): bigint => memory.getBigUint64(ptr + offset_userdata, true);

	export function create(memory: DataView, ptr: ptr): subscription {
		return {
			get userdata(): userdata { return getUserdata(memory, ptr); },
			get u(): subscription_u { return Subscription_u.create(memory, ptr + offset_u); }
		};
	}
}
