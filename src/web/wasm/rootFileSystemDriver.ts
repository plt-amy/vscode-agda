...
import { Errno, Filetype, Rights, WasiError, fd, fdflags, fdstat, filestat, fstflags, inode, lookupflags, oflags, rights } from './wasi';
...
class VirtualRootFileSystem {
	...
	public addMountPoint(filepath: string, deviceDriver: FileSystemDeviceDriver): void {
		...
		for (let i = 0; i < segments.length; i++) {
			if (current.kind === NodeKind.MountPoint) {
				throw new Error(`Cannot create virtual folder over mount point: /${segments.slice(0, i + 1).join("/")}`);
			}
	...
	public makeVirtualPath(deviceDriver: FileSystemDeviceDriver, filepath: string): string | undefined {
		...
		return path.join(nodePath, filepath);
	}
	...
	private getPath(inode: Node): string {
		...
		return path.join(...parts.reverse());
	}
}
